...
[nav1] = arch3_imu2cm_filter_nomag(imu1);
[nav2] = arch3_imu2cm_filter_nomag(imu2);
[nav3] = arch3_imu2cm_filter_nomag(imu3);
[nav4] = arch3_imu2cm_filter_nomag(imu4);

% Fusion of the four measurements
[navCM] = attitude_average_arch3(nav1, nav2, nav3, nav4);
...
sgtitle('Quaternions. Architecture 3.','interpreter','latex')

subplot(4,1,1)
plot(navCM.t, navCM.qua(:,1), 'og')
hold on
...
fh = figure(2);
% fh.WindowState = 'maximized';
sgtitle('Euler angles.  Architecture 3.','interpreter','latex')

subplot(3,1,1)
plot(navCM.t, navCM.roll, 'og')
hold on
