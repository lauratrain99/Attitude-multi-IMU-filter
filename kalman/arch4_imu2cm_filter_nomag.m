function [nav, kf] = arch4_imu2cm_filter_nomag(imu1, imu2, imu3, imu4)
%
...
nav.K  = zeros(N, 288);          % Kalman gain matrices, K
nav.S  = zeros(N, 144);           % Innovation matrices, S
nav.ob = zeros(N, 1);           % Number of observable states at each acceleromter data
nav.deltar = zeros(N, 12);        % Error residual
...
kf.deltar = zeros(12,1);
kf.deltar(1:3) = [-DCMbn1*av_1 - g_n]';
kf.deltar(4:6) = [-DCMbn2*av_2 - g_n]';
kf.deltar(7:9) = [-DCMbn3*av_3 - g_n]';
kf.deltar(10:12) = [-DCMbn4*av_4 - g_n]';
...
kf.R = diag([imu1.a_std, imu2.a_std, imu3.a_std, imu4.a_std]).^2;
...
for i = 2:N
      ...
      % KALMAN FILTER      
          ...
          nav.K(i,:)             = reshape(kf.K, 1, 288);
          nav.S(i,:)             = reshape(kf.S, 1, 144);
          nav.ob(i,:)            = ob;
